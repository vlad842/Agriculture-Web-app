                    ...
                    for (int year = 2013; year < 2018; year++)
                    {
                        for (int j = 1; j <= 2; j++)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(current_crop_name))
                            {
                                ...
                                {
                                    Id = m_SiteByYearID++,
                                    CurrentCropId = current_crop.ID,
                                    CurrentSiteId = current_site.Id,
                                    CurrentLayerId = layers_dicitonary[year].Id
                                };

                                current_crop.SiteByYearIDs.Add(current_site_by_year.Id);
                                current_site.SiteByYearsIds.Add(current_site_by_year.Id);
                                layers_dicitonary[year].SiteByYearsIds.Add(current_site_by_year.Id);
